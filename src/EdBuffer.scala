...
class EdBuffer {
    ...
    private var clipboard = new PlaneText()
    ...
    def deleteRange(pos: Int, len: Int) {
        ...
        setModified()
        if(pos<mark) mark -= len
    }
    def transpose
    {
        require(point<length, "Can't transpose the end of the file")
        point-=1
        val c1 = charAt(point)
        val c2 = charAt(point+1)
        deleteChar(point)
        deleteChar(point)
        insert(c1)
        insert(c2)
    ...
    def copy
    {
        val start = if (mark<point) mark else point
        val length = if (mark<point) point-mark+1 else mark-point+1
        text.getRange(start, length, clipboard)
    }
    def cut
    {
        val start = if (mark<point) mark else point
